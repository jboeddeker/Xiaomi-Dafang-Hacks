...
/system/sdcard/bin/mosquitto_sub.bin -v -h "$HOST" -p "$PORT" -u "$USER" -P "$PASS" -t "${TOPIC}/control/#" ${MOSQUITTOOPTS} | while read -r line ; do
  ...
  "${TOPIC}/control/motion/send_mail/set ON")                                                                                                                                                         
   ...
   "${TOPIC}/control/motion/send_telegram")                                                                                                                                                                
  ...
  "${TOPIC}/control/motion/send_telegram/set ON")                                                                                                                                                         
      motion_send_mail on                                                                                                                                                                       
      /system/sdcard/bin/mosquitto_pub.bin -h "$HOST" -p "$PORT" -u "$USER" -P "$PASS" -t "${TOPIC}"/motion/send_telegram ${MOSQUITTOPUBOPTS} ${MOSQUITTOOPTS} -m "$(motion_send_telegraml status)"      
    ;;                                                                                                                                                                                          
                                                                                                                                                                                                
    "${TOPIC}/control/motion/send_telegram/set OFF")                                                                                                                                                        
      motion_send_mail off                                                                                                                                                                      
      /system/sdcard/bin/mosquitto_pub.bin -h "$HOST" -p "$PORT" -u "$USER" -P "$PASS" -t "${TOPIC}"/motion/send_telegram ${MOSQUITTOPUBOPTS} ${MOSQUITTOOPTS} -m "$(motion_send_telegram status)"      
    ;;           
